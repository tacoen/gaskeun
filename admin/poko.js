...
function poko_tool(t) {
	var s = t.getAttribute('data-ref');
	var d = t.getAttribute('data-dst')
	var sc = document.querySelector('input[name="styles'+s+'"]');
	console.log(s,sc.value,t.innerHTML)
	what = s.replace(/\]\[/gi,'-').replace(/\[|\]/gi,'');
	var menus =
		"<a onclick='javascript:poko_modif(\"ori\",\""+s+"\",\""+d+"\");'>"+what+"</a>"+
		"<a onclick='javascript:poko_modif(\"a1\",\""+s+"\",\""+d+"\");'>15</a>"+
		"<a onclick='javascript:poko_modif(\"a2\",\""+s+"\",\""+d+"\");'>-15</a>"+
		"<a onclick='javascript:poko_modif(\"t1\",\""+s+"\",\""+d+"\");'>120</a>"+
		"<a onclick='javascript:poko_modif(\"t2\",\""+s+"\",\""+d+"\");'>270</a>"+
		"<a onclick='javascript:poko_modif(\"co\",\""+s+"\",\""+d+"\");'>180</a>";

	t.parentElement.innerHTML=menus;
}
	

function poko_copy(d,s) {
	var sc='input[name="styles'+s+'"]';
	var ds='input[name="styles'+d+'"]';
	document.querySelector(ds).value=document.querySelector(sc).value;
	document.querySelector(ds).click();
	document.querySelector(ds).focus();
}

function poko_modif(typ,s,d) {
	var sc='input[name="styles'+s+'"]';
	var ds='input[name="styles'+d+'"]';
	var obj = document.querySelector(sc)
	var color = obj.value;
/*	
	if (obj.getAttribute('data-original')===null) {
		var color = obj.value;
		obj.setAttribute('data-original',color)
		console.log('baru',color)
	} else {
		color = obj.getAttribute('data-original');
...
*/	
	...
	color = c.toHexString();
	console.log(color,c)
	document.querySelector(ds).value = color;
	document.querySelector(ds).click();
	document.querySelector(ds).focus();
}
